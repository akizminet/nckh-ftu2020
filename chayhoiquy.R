...
dulieu$DUMMYOWN_FR <- ifelse(dulieu$OWN_FR>0,1,0)
dulieu$DUMMYOWN_MNG <- ifelse(dulieu$OWN_MNG>0,1,0)
dulieu$DUMMYOWN_STATE <- ifelse(dulieu$OWN_STATE>0,1,0)
dulieu$DUMMYIND <- ifelse(dulieu$IND>0,1,0)
dulieu$YEAR <- factor(dulieu$YEAR)
...
Q1RETURN <- quantile(dulieu$RETURN,0.25)
Q3RETURN <- quantile(dulieu$RETURN,0.75)
IQRRETURN <- Q3RETURN-Q1RETURN
Q1NCSKEW <- quantile(dulieu$NCSKEW,0.25)
Q3NCSKEW <- quantile(dulieu$NCSKEW,0.75)
IQRNCSKEW <- Q3NCSKEW-Q1NCSKEW
temp <- dulieu[(Q1RETURN-1.5*IQRRETURN)<=RETURN & RETURN <= (Q3RETURN+1.5*IQRRETURN)]

modelNCSKEWtemp <- lm(NCSKEWT~OWN_STATE+OWN_FR+OWN_MNG+BOARD_SIZE+IND+DUAL+BIG4+AO+log(MARKET_VALUE)+MB+DA+ROE+NCSKEW+SIGMA+DTURN+RETURN,temp)
modelDUVOLtemp <- lm(DUVOLT~OWN_STATE+OWN_FR+OWN_MNG+BOARD_SIZE+IND+DUAL+BIG4+AO+log(MARKET_VALUE)+MB+DA+ROE+NCSKEW+SIGMA+DTURN+RETURN,temp)
summary(modelNCSKEWtemp)
summary(modelDUVOLtemp)
